...
const BlogCardOne = (
    ...
    const new_date:any = date?new Date(date):null
    return (
            ...
            <div className="mt-6">
                <ul className="flex items-center text-[13px] font-medium text-dark-2">
                    <li className="flex items-center relative first:pl-0 pl-2 pr-2 before:content-[''] before:absolute before:h-2/3 before:w-[1px] before:bg-dark-2 before:-translate-y-1/2 before:top-1/2 before:left-0 first:before:hidden">
                        ...
                        <span className="ml-2">{new_date.toLocaleDateString('en-US', date_options)}</span>
                    </li>
                </ul>
                ...
                <Link href={`/blog/${slug}`} className="group ease-in hover:text-primary-1 duration-200 inline-flex items-center mt-4 lg:text-base text-sm text-dark-1 font-medium">
