    ...
    useEffect(() => {
        ...
        async function GetData() {
            ...
            const response_categorys = await fetch(
                `${process.env.NEXT_PUBLIC_SITE_URL}/api/categorys_function?token=${process.env.NEXT_PUBLIC_BEARER_TOKEN}`,
                {
                  //   cache: 'force-cache',
                    method: 'GET',
                    headers : {
            ...
            if (totalcount != 0) setCategories(get_categorys)
            const response_regions = await fetch(
                `${process.env.NEXT_PUBLIC_SITE_URL}/api/regions_function?token=${process.env.NEXT_PUBLIC_BEARER_TOKEN}`,
                {
                  //   cache: 'force-cache',
                    method: 'GET',
                    headers : {
            ...
            let get_regions:any = await response_regions.json()
            totalcount = get_regions?.pagination?.total || 0;
            if (totalcount != 0) setDestinations(get_regions)
        }
        GetData()
                        ...
                        <ul className="flex items-center nav-list">
                            ...
                            {destinations.data && 
                                <li className="group/mega-dropdown">
                                    <Link href="/destinations" className="nav-link has-dropdown">Destination</Link>
                                    <div className="nav-mega-dropdown grid grid-cols-6 gap-x-1">
                                        <div className="col-span-4 border-r">
                                            <Link href="/destinations" className="py-2 flex items-center border-b border-stock-1 text-base text-dark-1 hover:text-primary-1 duration-200">
                                            ...
                                            <ul className="grid grid-cols-3 mega-dropdown-links">
                                                {destinations?.data?.regions_data.slice(0, 15).map((item:any, index:number) => (
                                                    <>
                                                        <li><Link href={`/destinations/${item.json?.seo?.slug}`}>{item.json?.detail?.name}</Link></li>
                                                    </>
                                                ))}
                                            </ul>
                                        </div>
                            ...
                            {categories.data && 
                                <li className="group/mega-dropdown">
                                    <Link href="/villa-type" className="nav-link has-dropdown">Villa Type</Link>
                                    <div className="nav-mega-dropdown grid grid-cols-6 gap-x-1">
                                        <div className="col-span-4 border-r">
                                            <Link href="/villa-type" className="py-2 flex items-center border-b border-stock-1 text-base text-dark-1 hover:text-primary-1 duration-200">
                                                Browse All Type
                                                <div className="max-w-[20px] ml-2">
                                                    <svg className="group-hover/btn:translate-x-2 duration-200 " width={27} height={14} viewBox="0 0 27 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M0.217443 6.25H18.4827C18.6276 6.25 18.7001 6.30263 18.7001 6.40789V7.59211C18.7001 7.69737 18.6276 7.75 18.4827 7.75H0.217443C0.0724811 7.75 0 7.69737 0 7.59211V6.40789C0 6.30263 0.0724811 6.25 0.217443 6.25Z" fill="currentColor" />
                                            ...
                                            <ul className="grid grid-cols-2 mega-dropdown-links">
                                                {categories?.data?.categorys_data?.slice(0, 10).map((item:any, index:number) => (
                                                    <>
                                                        <li><Link href={`/villa-type/${item.json?.seo?.slug}`}>{item.json?.detail?.name}</Link></li>
                                                    </>
                                                ))}
                                            </ul>
                                        </div>
                            ...
                            {categories.data && destinations.data && 
                                <li>
                                    <Link href="/holiday-villas" className="nav-link">Holiday Villas</Link>
                                </li>
                            }
                            <li>
