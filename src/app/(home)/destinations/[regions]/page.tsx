...
export const metadata: Metadata = {
    title: 'Destinations | Arid - Travel & Tourism HTML/Tailwind CSS Template',
...
const DestinationsParent = async ({
    ...
    const slug = (await params).regions
    const page_name = `destinations/${slug}`
    let totalcount = 0;
    const response_regions = await fetch(
        `${process.env.SITE_URL}/api/regions_function?slug=${slug}&token=${process.env.NEXT_PUBLIC_BEARER_TOKEN}`,
        {
          //   cache: 'force-cache',
            method: 'GET',
            headers : {
    ...
    let get_destinations:any = await response_regions.json()
    totalcount = get_destinations?.pagination?.total || 0;
    if (totalcount == 0){ 
        notFound()
    }
    const regions_id = get_destinations.data.regions_data[0].id
    const regions_up_id = get_destinations.data.regions_data[0].json.regions_up
    const regions = [regions_id]
    const regions_up = regions_up_id
    ...
    const response = await fetch(
        `${process.env.SITE_URL}/api/villas_function?take=${take}&skip=${skip}&regions=${regions}&token=${process.env.NEXT_PUBLIC_BEARER_TOKEN}`,
        {
          //   cache: 'force-cache',
            method: 'GET',
            headers : {
                    ...
                    <div className="grid grid-cols-12 lg:gap-12 gap-base">
                        ...
                        <div className="lg:col-span-4 col-span-12 ">
                            <PackageSidebarFilter 
                                params={{regions : String(regions_id)}}
                            />
                        </div>
                    </div>
                    {pagination > 1 && (
